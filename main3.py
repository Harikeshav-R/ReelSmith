...
def research_node(state: State) -> State:
    # search_engine = SearXNGResearch(GoogleLLM("gemini-2.0-flash"), "Summarize this article in 200 words:")
    search_engine = SearXNGResearch(OllamaLLM("qwen3:8b"), "Summarize this article in 200 words:")
    state = asyncio.run(search_engine.research(state)).model_dump()
    return state


def script_node(state: State) -> State:
    # script_generator = ScriptGenerator(GoogleLLM("gemini-2.0-flash"), "You are a video scriptwriter.")
    script_generator = ScriptGenerator(OllamaLLM("qwen3:8b"), "You are a video scriptwriter.")
    state = script_generator.generate(state).model_dump()
    return state
...
if __name__ == "__main__":
    ...
    for i in result.search_summary:
        print("\n\n\n")
        print(i)
